...
namespace Translator.Crypto
{
    public class BlowFishIntVer
    {
        ...
        public BlowFishIntVer(string hexKey)
        {
        ...
        public BlowFishIntVer(byte[] cipherKey)
        {
            randomSource = new RNGCryptoServiceProvider();
            SetupKey(cipherKey);
        }
        ...
        private byte[] Crypt_ECB(byte[] text, bool decrypt)
        {
            ...
            int paddedLen = (text.Length % 8 == 0 ? text.Length : text.Length - (text.Length % 8));
            byte[] plainText = new byte[paddedLen];
            //Buffer.BlockCopy(text, 0, plainText, 0, text.Length);
            Buffer.BlockCopy(text, 0, plainText, 0, paddedLen);
            int razn = text.Length - paddedLen;
            byte[] ost = null;
            if (razn != 0)
            {
                ost = new byte[razn];
                Buffer.BlockCopy(text, paddedLen, ost, 0, razn);
            }
            ...
            byte[] ntext = new byte[text.Length];
            Buffer.BlockCopy(plainText, 0, ntext, 0, paddedLen);
            if (razn != 0)
            {
                Buffer.BlockCopy(ost, 0, ntext, paddedLen, razn);
            }
            return ntext;
        }
        ...
        /// </summary>
        private void encipher(bool init)
        {
            xl_par ^= bf_P[0];
